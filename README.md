...
(1) Create a folder `routes`, and a file `userRoutes.js`.
    It is fine if we put all the logic in userRoutes.js, but it is a good practice to have logic in **controllers**.
...
Of course we can use try...catch blocks, but that would be repetitive. Async handler is a wrapper around each controller function for error handling.
...
Usage - use `asyncHandler()` to wrap the controller function. It will allow us to use async/await.
...
(3) Create error handlers in   `backend/middleware/errorMiddleware.js`
...
(2) Create connection function in `backend/config/db.js`
```
...
**Note:** Add the following lines to `server.js` in order to parse JSON (e.g. req.body) and send form data.
```
...
(2) Generate JWT
    **Note:**
    As both registerUser and authUser will use JWT, we put the function in `backend/utils/generateToken.js`.
```
...
`backend/controllers/userController.js`: look for the user using `req.user._id` (which is made available in `authMiddleware`) in database, update name/email/password as requested, save the updated user. If no user is found, throw an error.
...
c. tagTypes has to do with caching so we do not have to fetch data everytime.
